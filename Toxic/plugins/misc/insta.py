...
async def insta_download(client: Client, message: Message):
    ...
    try:
        ...
        if not results:
            return await processing_message.edit("Thik se To link Dala hai na Bsdk Bcz No media found.")
        ...
        for item in results:
            ...
            if ".mp4" in download_link:
                ...
                await message.reply_text(f"Sorry Babu,Unsupported media type: {download_link}")

    except Exception as e:
        await processing_message.edit(f"404 Error: {e} Contact Owner @szxns")
    finally:
